...
public class FastShepherd extends FastCalculatorBase {
    ...
    public double getValue(FastChain chain, LinkUp linkUp) throws Exception {
        double T1 = getPosCount(chain.getFastUniformChain("T"), 0);
        double T2 = getPosCount(chain.getFastUniformChain("T"), 1);
        double T3 = getPosCount(chain.getFastUniformChain("T"), 2);
        double Tp = max(T1, T2, T3) / (min(T1, T2, T3) + 1);
        double C1 = getPosCount(chain.getFastUniformChain("C"), 0);
        double C2 = getPosCount(chain.getFastUniformChain("C"), 1);
        double C3 = getPosCount(chain.getFastUniformChain("C"), 2);
        double Cp = max(C1, C2, C3) / (min(C1, C2, C3) + 1);
        double A1 = getPosCount(chain.getFastUniformChain("A"), 0);
        double A2 = getPosCount(chain.getFastUniformChain("A"), 1);
        double A3 = getPosCount(chain.getFastUniformChain("A"), 2);
        double Ap = max(A1, A2, A3) / (min(A1, A2, A3) + 1);
        double G1 = getPosCount(chain.getFastUniformChain("G"), 0);
        double G2 = getPosCount(chain.getFastUniformChain("G"), 1);
        double G3 = getPosCount(chain.getFastUniformChain("G"), 2);
    ...
    private double getPosCount(FastUniformChain chain, int pos) throws Exception {
        HashSet<Integer> poses = new HashSet<Integer>();
        poses.add(pos);
        return FastCalculatorFactory.getPositionedEventCount(poses, 3).getValue(chain, LinkUp.Start);
    }
